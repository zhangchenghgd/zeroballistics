...
def writeMeshes(obj, file):
    ...
    for face_ind, f in enumerate(mesh.faces):
        ...
        if face_ind % PROGRESS_UPDATE_INTERVAL == 0:
            DrawProgressBar(float(face_ind) / len(mesh.faces),
    ...
    DrawProgressBar(1.0, "BBM: " + mesh.name)

    DrawProgressBar(0.0, "BBM (writing data): " + mesh.name)
    ...
    for (mat,tex_images), cur_info in mesh_info.iteritems():
        ...
        message("%s : %d vertices, %d faces exported" %
                ...
                msg.V_DEBUG)
    DrawProgressBar(1.0, "BBM (writing data): " + mesh.name)
...
def exportBbm(quick_n_dirty = False):
    ...
    filename = os.path.join(config["export_dir"], "models",
                            getCanonicalName(Scene.GetCurrent().getName()) + ".bbm")
    ...
    for o in bbm_objects:
        if getCanonicalName(o.getName()) == "PROPS": raise Exception("PROPS exported as bbm! Aborting...")
    ...
    props_object = getPropsObject()
    flags=0
    if getPropertySafe(props_object, "Blocker")  : flags += BNO_SHADOW_BLOCKER
    if getPropertySafe(props_object, "Instanced"): flags += BNO_INSTANCED
    if getPropertySafe(props_object, "Receiver") : flags += BNO_SHADOW_RECEIVER
    writeUnsigned16(fid, flags)

    lod_class = getPropertySafe(props_object, "LodClass")

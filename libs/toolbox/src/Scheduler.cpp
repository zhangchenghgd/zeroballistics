...
void Scheduler::frameMove(float dt)
{
    ...
    if (!task_.empty())
    {
        ...
        while (cur_task = &(*task_.begin()),
               cur_task->next_time_ < cur_time_+dt)
        {
            ++tasks_per_frame;
...
{
    s_log << Log::debug('t') << "Adding task " << name << " with FPS " << 1.0f / period <<"\n";
                          ...
                          RegisteredFpGroup * group)
{
    ...
    inserted->next_time_       = cur_time_ + delay;
...
void Scheduler::reschedule(hTask task, float new_time)
{
         ...
         ++it)
    {
        if (it->id_ == task)
        {
            ...
            {
                it->next_time_ = cur_time_ + new_time;
                reschedule(it);
            }

...
bool ServerInfo::readFromBitstream(RakNet::BitStream & stream)
{
    ...
    ret &= stream.Read(version_.game_);
 ...
 */
void ServerInfo::writeString(RakNet::BitStream & stream, std::string str) const
{
 ...
 *  String compressor doesn't work when communicating with openbsd ference??
 */
bool ServerInfo::readString (RakNet::BitStream & stream, std::string & str) const
{
    bool ret = true;

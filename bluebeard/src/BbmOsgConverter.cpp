...
void NodeUserData::addLodsIfNeccessary()
{
    ...
    {
        if (isGroupMember("lod" + toString(i+1)))
        {
            has_lod = true;
            break;
        }
    ...
    if (!has_lod)
    {
        ...
        for (unsigned i=0; i<NUM_LOD_LVLS; ++i)
        {
            groups_.back().push_back("lod"+toString(i+1));
        }
    }
}
...
//------------------------------------------------------------------------------
CreateShaderVisitor::CreateShaderVisitor() :
...
//------------------------------------------------------------------------------
void CreateShaderVisitor::apply (osg::MatrixTransform & node)
{
    RootNodeUserData * ud = dynamic_cast<RootNodeUserData*>(node.getUserData());
    if (ud)
    {
        root_node_flags_ = ud->flags_;
    }
    traverse(node);
}
...
//------------------------------------------------------------------------------
void CreateShaderVisitor::apply (osg::Geode & geode)
{
    for (unsigned g=0; g<geode.getNumDrawables(); ++g)
    {
        osg::Geometry * cur_geometry = dynamic_cast<osg::Geometry*>(geode.getDrawable(g));
        assert(cur_geometry);        
        
        GeometryUserData * mesh_ud = dynamic_cast<GeometryUserData*>(cur_geometry->getUserData());
        if (!mesh_ud) continue;

        if (mesh_ud->flags_ & bbm::BMO_ALPHA_BLEND) continue;
    ...
    mesh_flags_mask_(0)
{
    ...
    {
        node_flags_mask_ |= bbm::BNO_SHADOW_RECEIVER | bbm::BNO_SHADOW_BLOCKER;
    }
    ...
    node_flags_mask_ = ~node_flags_mask_;
}
...
void BbmOsgVisitor::apply(bbm::Node* node)
{
    ...
    data->flags_    = node->getFlags() & node_flags_mask_;
...
{
    EffectNode * eff_node = s_effect_manager.createEffect(
        effect->getName(),
        effect->getTransform().getTranslation(),
        -effect->getTransform().getZ(),
        true);
    
    cur_node_.top()->addChild(eff_node);
    createGroupUserData(effect, eff_node);
...
void BbmOsgVisitor::createStateSet(const bbm::Mesh * mesh, osg::Geometry * geom)
{
    ...
    {
        StatesetMap::iterator it = state_set_cache_.find(mesh->getUniqueMaterialIdentifier(mesh_flags_mask_));
    ...
    if (s_params.get<bool>("client.graphics.cache_statesets"))
    {    
        state_set_cache_[mesh->getUniqueMaterialIdentifier(mesh_flags_mask_)] = state_set;
...
void BbmOsgVisitor::createGeometry(const bbm::Mesh * mesh, osg::Geometry * geom)
{
    ...
    // Tangent & bitangetn, if existent
    // create normals.
    if (mesh->getTangentData().size())
    {

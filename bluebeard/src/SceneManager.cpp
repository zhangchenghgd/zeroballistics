...
void SceneManager::init()
{
   ...
   // ->setRenderStage(new osgUtil::RenderStage(osgUtil::RenderBin::SORT_FRONT_TO_BACK));
    ...
    light_->setDiffuse(osg::Vec4(1,1,1,1));
    ...
    // Lighting is done completely in shaders, so disable it here. For
    // non-shader mode, it is enabled per model.
...
void SceneManager::checkCapabilities(bool verbose)
{
    ...
    {
        s_log << Log::warning
              << "Your graphics card or driver supports only less than 6 texture units.\n";
        shaders_enabled_ = false;
    }
    ...
    {
        s_log << Log::warning
              << "Shader Model 2.0 is required for best visual quality, but only "
              << (shader_model ? toString(shader_model) : std::string("1.0 or lower"))
              << " is supported.\n";
        shaders_enabled_ = false;
    }
    ...
    {
        s_log << Log::warning
              << "GLSL version 1.10 is required for optimal visual quality, but only "
    ...
    if (!shaders_enabled_)
    {
        s_params.set<bool>("client.shadows.enabled", false);
        s_params.set<unsigned>("client.graphics.shader_quality", 0);
    }
    


    // ---------- framebuffer object extension for shadows ----------
    ...
    // needed to blend out object parts.
    if (!osg::isGLExtensionSupported(getContextId(), "GL_ARB_imaging"))
    {
        s_log << Log::warning
              << "Your driver doesn't support the GL_ARB_imaging extension. "
              << "Prepare for visual artifacts.\n";
    }
}


//------------------------------------------------------------------------------
bool SceneManager::areShadersEnabled() const
{
    return shaders_enabled_;
}

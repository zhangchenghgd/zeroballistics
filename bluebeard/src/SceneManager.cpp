...
void SceneManager::printNodeHierarchy(osg::Node* node, unsigned depth, std::vector<unsigned> print_pipe)
{
    ...
    if (geode)
    {
        ...
        {
            s_log << geode->getDrawable(d)->className() << " ";
                                              ...
                                              bool * reused)
{
    ...
    if (s_params.get<bool>("client.graphics.reflections")) {defines += "#define REFLECTIONS\n";}
...
void SceneManager::checkCapabilities(bool verbose)
{
    ...
    if(!osg::isGLExtensionSupported(getContextId(), "GL_EXT_framebuffer_object"))
    {
        ...
        s_params.set("client.graphics.reflections", false);
        
    } else
    {
        if(getOpenGlRenderer() == "ati")
        {
            if (verbose)
            {        
                s_log << Log::warning
                      << "gl_ClipVertex is not supported (ATI card). Reflections disabled.\n";
            }
            s_params.set("client.graphics.reflections", false);
        }

...
unsigned NetworkCommand::num_bits_sent_[];
...
void writeToBitstream(RakNet::BitStream & stream, std::string str)
{
    ...
    StringCompressor::Instance()->EncodeString(const_cast<char*>(str.c_str()), // const bug in raknet RRRR
                                               str.length()+1,
                                               &stream);    
}
...
bool readFromBitstream(RakNet::BitStream & stream, std::string & str)
{
    ...
    if (!StringCompressor::Instance()->DecodeString(&tmp[0],
                                                    len+1,
                                                    &stream)) return false;
    str = &tmp[0];
...
//------------------------------------------------------------------------------
void NetworkCommand::logAccountingInfo()
{
    TimeValue cur_time;
    getCurTime(cur_time);

    float passed_secs = getTimeDiff(cur_time, s_log.getStartTime()) / 1000.0f;
    ...
    {
        float kb = (float)num_bits_sent_[i]/8/1024;        
        total += kb;
    }
...
//------------------------------------------------------------------------------
void NetworkCommand::accountPacket(RakNet::BitStream & stream)
{
    ...
    num_bits_sent_[packet_id - TPI_FIRST] += stream.GetNumberOfBitsUsed();
}

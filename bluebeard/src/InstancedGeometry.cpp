...
InstancedPrimitive::InstancedPrimitive(const osg::Geometry & geometry, const InstancingInfo & lod_info) :
    ...
    batch_uniform_buffer_(s_params.get<unsigned>("client.graphics.instance_batch_size"))
{
    ...
    getStateSet()->addUniform(new osg::Uniform(osg::Uniform::FLOAT_VEC3,
                                               ...
                                               NUM_VEC3S_PER_INSTANCE*batch_uniform_buffer_.size()));
}
...
void InstancedPrimitive::drawImplementation(osg::RenderInfo& renderInfo) const
{
    ...
    if (batch_uniform_buffer_.empty()) return;
    ...
    unsigned num_full_batches = info_.instance_data_.size() / batch_uniform_buffer_.size();
    for (unsigned batch=0; batch < num_full_batches; ++batch)
    {
        memcpy(&batch_uniform_buffer_[0],
               &info_.instance_data_[batch*batch_uniform_buffer_.size()],
               batch_uniform_buffer_.size()*sizeof(InstanceData));

        gl2_extensions_->glUniform3fv(instance_data_uniform_location_,
                                      batch_uniform_buffer_.size()*NUM_VEC3S_PER_INSTANCE,
                                      (GLfloat*)&batch_uniform_buffer_[0]);
        draw_elements_->drawCapped(state, 0);        
    }

    unsigned num_remaining_elements = info_.instance_data_.size() - num_full_batches*batch_uniform_buffer_.size();
    if (num_remaining_elements)
    {
        memcpy(&batch_uniform_buffer_[0],
               &info_.instance_data_[num_full_batches*batch_uniform_buffer_.size()],
               num_remaining_elements*sizeof(InstanceData));

        gl2_extensions_->glUniform3fv(instance_data_uniform_location_,
                                      batch_uniform_buffer_.size()*NUM_VEC3S_PER_INSTANCE,
                                      (GLfloat*)&batch_uniform_buffer_[0]);
        draw_elements_->drawCapped(state, draw_elements_->size() / batch_uniform_buffer_.size() * num_remaining_elements);
    }
...
void InstancedPrimitive::accept (osg::PrimitiveFunctor & pf) const
{
    ...
    for (unsigned i=0; i< info_.instance_data_.size()/batch_uniform_buffer_.size(); ++i)
    {
        Geometry::accept(pf);
    }
}
...
void InstancedPrimitive::replicateGeometry(const osg::Geometry & geometry)
{
    ...
    unsigned total_num_vertices = batch_uniform_buffer_.size() * num_vertices;
    ...
    draw_elements_ = new DrawElementsUIntCapped(src_draw_elements.getMode(),
                                                src_draw_elements.size()*batch_uniform_buffer_.size());

    for (unsigned b=0; b<batch_uniform_buffer_.size(); ++b)
    {
    ...
    setVertexArray(     replicateArray<osg::Vec3Array>(geometry.getVertexArray(),    batch_uniform_buffer_.size()));
    setNormalArray(     replicateArray<osg::Vec3Array>(geometry.getNormalArray(),    batch_uniform_buffer_.size()));
    setTexCoordArray(0, replicateArray<osg::Vec2Array>(geometry.getTexCoordArray(0), batch_uniform_buffer_.size()));

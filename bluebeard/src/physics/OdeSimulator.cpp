    ...
    num_dynamic_bodies_(0)
{
    ...
    s_console.addVariable(("num_dynamic_bodies_" + name).c_str(),
                          &num_dynamic_bodies_,
                          &fp_group_);
}
...
void OdeSimulator::frameMove(float dt)
{
    ...
    // Collide actor space first. Otherwise, objects might wake up in
    // response to a collision event and will collide again when back
    // in actor space.
    actor_space_->collide(NULL,          false);
    actor_space_->collide(static_space_, true );
...
//------------------------------------------------------------------------------
unsigned OdeSimulator::getNumDynamicBodies() const
{
    return num_dynamic_bodies_;
}
...
void OdeSimulator::handleBodyVelocities()
{
         ...
         ++it)
    {
        ...
        if (cur_body->isSleeping()) continue;

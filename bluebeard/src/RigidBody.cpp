...
void RigidBody::createProxy()
{
    ...
    target_object_->changeSpace(target_object_->getSimulator()->getActorSpace(), NULL);
...
{
    if (proxy_object_ && !proxy_object_->isStatic())
    {
        if (proxy_object_->isSleeping() && !target_object_->isSleeping())
        {
 ...
 *  in game_state.
 */
void RigidBody::readInitValuesFromBitstream(RakNet::BitStream & stream, GameState * game_state)
{
    PROFILE(RigidBody::readInitValuesFromBitstream);
    
    GameObject::readInitValuesFromBitstream(stream, game_state);    
    
    createTargetObject(name_, game_state->getSimulator());

    if (!isStatic()) createProxy();

    init();

    emit(RBE_INITIALIZATION_FINISHED);

    readStateFromBitstream(stream, OST_BOTH, 0);
...
//------------------------------------------------------------------------------
/**
 *  Creates a RigidBody object from a description file. If the
 *  specified description file doesn't exist, a type of RigidBody is
 *  assumed and desc_file is used as bbm and xml file name.
 *
 *  \param desc_file The description file to load the RigidBody
 *  from. This file contains the type of the object, its xml,bbm file
 *  names and optionally additional type specific parameter sections.
 */
...
//------------------------------------------------------------------------------
void RigidBody::readCoreState(RakNet::BitStream & stream, uint32_t timestamp)
{
    bool quantized;
    stream.Read(quantized);
...
void RigidBody::warpProxy(bool force)
{
    ...
    if (force ||
        (target_object_->getPosition() - proxy_object_ ->getPosition()).lengthSqr() > 0.1f) // PPPP
    {

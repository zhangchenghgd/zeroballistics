...
/// Determines the maximum steepness of the terrain for grass elements
/// to be still placed.
const float TERRAIN_NORMAL_THRESHOLD = cosf(deg2Rad(35.0f));
    ...
    for (unsigned r=0; r<height; ++r)
    {
        for (unsigned c=0; c<width; ++c)
        {
            ...
            for (unsigned i=0; i<instances_per_cell_; ++i)
            {
                ...
                if (n.y_ < TERRAIN_NORMAL_THRESHOLD)
                {
                                            ...
                                            const std::vector<bbm::DetailTexInfo> & tex_info) const
{
    ...
    for (unsigned i=0; i<instance_probability.size(); ++i)
    {
        ...
        for (unsigned j=0; j<instance_probability[i].size(); ++j)
        {
            ...
            instance_probability[i][j] *= density / (max_density * sum);
        }
    }
}

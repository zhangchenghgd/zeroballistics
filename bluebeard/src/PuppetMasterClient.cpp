    ...
    hud_(new GameHud(this))
{
    ...
    s_input_handler.registerInputCallback("Fire Cannon",
                                          ...
                                          &fp_group_);
    s_input_handler.registerInputCallback("Fire MG",
                                             ...
                                             const SystemAddress & pid1)
{
    ...
    case network::SMT_CHAT:
        if (pid1 == UNASSIGNED_SYSTEM_ADDRESS)
        {
            ...
            s_log << "Server: "
                  << message
                  << "\n";
            hud_->addChatLine("Server: " + message, Color(0.5f,0.0f,9.0f));
        } else
        {
            // Player message
    ...
    case network::SMT_PLAYER_NAME:
        ...
        if(!player_name_changed->getName().empty()
           && player_name_changed != (&local_player_))
        {
            hud_->addMessage(player_name_changed->getName() + " changed his name to " + message , Color(0.0f,1.0f,0.0f));
        }
...
void PuppetMasterClient::onRequestReady()
{
    ...
    hud_->addMessage("Press '" +
                     s_input_handler.getKeyForFunction("Show Help") +
                     "' for help.", Color(1.0f,0.0f,0.0f));
}
...
void PuppetMasterClient::deleteRemotePlayer(const SystemAddress & id)
{
         ...
         ++it)
    {
        if (it->getId() == id)
        {
            hud_->addMessage(it->getName() + " has left the game.", Color(1.0f,0.0f,0.0f));
...
void PuppetMasterClient::sendChat(const std::string &  msg)
{
    ...
    {
        hud_->addMessage("Blocked due to flooding.",Color(1.0,0.0,0.0,1.0));
        return;
    }
...
std::string PuppetMasterClient::changePlayerName(const std::vector<std::string> & args)
{
    ...
    if(reg_ex.match(name))
    {
        ...
        local_player_.setName(name);
        game_logic_->onPlayerNameChanged(&local_player_);

        network::SetPlayerNameCmd cmd(name);
        cmd.NetworkCommandClient::send(client_interface_);

...
//           << "\n";
    ...
    // XXX testing only: determine history size
...
//     s_log << Log::debug('b')
//           << "received correction " << (unsigned)sequence_number << "\n";
...
//           << "\n";
    ...
    if (seqNumberDifference(sequence_number, history_[hist_head_].sequence_number_) < 0 )
    {
        s_log << Log::debug('n')
    ...
    {
        s_log << Log::debug('b') << "discarding move "
              << (unsigned)history_[hist_head_].sequence_number_ << "\n";
...
//               << (unsigned)sequence_number << " matches.\n";
    ...
    do
    {
        ...
        } else
        {
            ...
            correct_controllable_->writeStateToBitstream(state, OST_CORE | OST_UNQUANTIZED);
...
void LocalPlayer::setControllable(Controllable * new_controllable)
{
    ...
    if (controllable_)
    {
        ...
        controllable_->getTarget()->changeSpace(controllable_->getTarget()->getSimulator()->getActorSpace(),
                                                NULL);
    ...
    if (controllable_)
    {
        correct_controllable_ = controllable_->clone(replay_simulator_.get());
        ...
        test_controllable_ = controllable_->clone(replay_simulator_.get());
        assert(!test_controllable_->getProxy());
        test_controllable_->getTarget()->changeSpace(test_controllable_->getTarget()->getSimulator()->getActorSpace(),
                                                     NULL);
...
void LocalPlayer::writeStateToHistory(const Controllable * c, unsigned index)
{
    ...
    c->writeStateToBitstream(stream, OST_CORE | OST_UNQUANTIZED);

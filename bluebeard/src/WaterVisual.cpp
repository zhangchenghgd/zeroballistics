...
void WaterVisual::operator() (osg::Node *node, osg::NodeVisitor *nv)
{
    ...
    projection_matrix = matGl2Osg(camera.getPerspectiveTransform()); 

    // if camera is above water plane
    if(camera.getPos().y_ > water_height_)
    {
        osg::Vec3 eye_pos = osg::Vec3(  camera.getPos().x_,
                                        (-camera.getPos().y_) + 2.0 * water_height_,
                                        camera.getPos().z_);

        osg::Vec3 eye_dir = osg::Vec3(eye_pos.x() - camera.getTransform().getZ().x_,
                                      eye_pos.y() + camera.getTransform().getZ().y_ ,
                                      eye_pos.z() - camera.getTransform().getZ().z_);
    ...
    ///< the code for oblique frustum culling starts here:
    {
        // define the water-plane in world space
        osg::Plane clip_plane(0.0,1.0,0.0,-water_height_);

        osg::Matrix MV = view_matrix;

        // transform plane into camera space
        clip_plane.transform(MV);


        ///< this is the oblique frustum culling code from Eric Lengyel
        ///< found at: http://www.terathon.com/code/oblique.php
        ///< transforms the near plane of the reflection camera into the water plane
        ///< therefore no user clipping plane is needed
        Matrix proj = matOsg2Gl(projection_matrix);

        osg::Vec4 Q = osg::Vec4(    (sign((float)clip_plane.asVec4().x()) + proj._13 ) / proj._11,
                                    (sign((float)clip_plane.asVec4().y()) + proj._23 ) / proj._22,
                                    -1.0,
                                    (1.0 + proj._33 ) / proj._34);


       // Calculate the scaled plane vector
        osg::Vec4 c = clip_plane.asVec4() * (2.0f / (clip_plane.asVec4() * Q));
        ...
        // write the modified projection matrix back to reflection camera
        projection_matrix = matGl2Osg(proj);   
    }
    ...
    osg::Matrix MVPT = osg_wrapper_->getOsgNode()->getMatrix() * view_matrix * projection_matrix *
    ...
    reflections_enabled_(false)
{
    ...
    reflections_enabled_ = s_params.get<unsigned>("client.graphics.water_quality") == 2;
}
...
void WaterVisual::onModelChanged()
{
    ...
    if (s_params.get<unsigned>("client.graphics.water_quality") != 0)
    {
    ...
    reflection_camera_->addChild(s_scene_manager.getShadowReceiverGroup());
    reflection_camera_->addChild(s_scene_manager.getDefaultGroup());
...
{
    if (s_params.get<unsigned>("client.graphics.water_quality") != 2) return;

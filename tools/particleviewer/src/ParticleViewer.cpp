...
void ParticleViewer::restartEffect(void* eff)
{
    try
    {
        ...
        osg::Group * new_effect = s_effect_manager.createEffect(effects_list_[ind]);
        *it = new_effect;
        new_effect->addObserver(this);
        s_scene_manager.addNode(new_effect);
    } catch (Exception &e )
    {
...
void ParticleViewer::loadEffect(const std::string & name)
{
    ...
    osg::Group * effect_node = s_effect_manager.createEffect(name);
    assert(effect_node);

    s_scene_manager.addNode(effect_node);

    effect_node->addObserver(this);
    effects_.push_back(effect_node);

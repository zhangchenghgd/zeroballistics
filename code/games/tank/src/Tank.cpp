...
//------------------------------------------------------------------------------
void Tank::readInitValuesFromBitstream(RakNet::BitStream & stream, GameState * game_state, uint32_t timestamp)
{
    Controllable::readInitValuesFromBitstream(stream, game_state, timestamp);
...
void Tank::writeStateToBitstream (RakNet::BitStream & stream, unsigned type) const
{
    ...
    if (type & OST_CORE)
    {
        ...
        stream.Write(target_steer_angle_);
#else
        network::writeAngleToBitstream(stream, target_yaw_);
        
        if (!doing_stabilization_) network::writeAngleToBitstream(stream, target_pitch_);
        else                       network::writeAngleToBitstream(stream, last_target_global_pitch_);        

        network::writeAngleToBitstream(stream, target_steer_angle_);
#endif
...
void Tank::handleProxyInterpolation()
{
    ...
    proxy_steer_angle_ += (target_steer_angle_   - proxy_steer_angle_)  * turret_proxy_interpolation_factor;
}
...
void Tank::setLocallyControlled(bool b)
{
    ...
    if (!b)
    {
        ...
        for(unsigned w=0; w < NUM_WEAPON_SLOTS; w++)
        {
            weapon_system_[w]->handleInput(false);
        }
    }
}

//------------------------------------------------------------------------------
Tank * Tank::cloneForReplay(physics::OdeSimulator * sim) const
{
    return new Tank(*this, sim);
}
...
//------------------------------------------------------------------------------
void Tank::getLauncherTransform(Matrix * res)
{
    Matrix tank = target_object_->getTransform();
    ...
    *res = tank * yaw * launcher_pos_;
}

//------------------------------------------------------------------------------
void Tank::getTargetTurretPos(float & yaw, float & pitch)const
{
    yaw   = target_yaw_;
    pitch = target_pitch_;
}
...
//------------------------------------------------------------------------------
void Tank::setHasRamUpgrade(bool b)
{
    params_.set("tank.ram_upgrade", false);
}
...
//------------------------------------------------------------------------------
float Tank::getProxySteerAngle() const
{
    return proxy_steer_angle_;
}
...
void Tank::loadParameters(const std::string & filename, const std::string & super_section)
{
    ...
    try
    {
        ...
        setArmorParams(hasRamUpgrade() ?
                       params_.get<float>("tank.front_armor_factor_ram") :
                       params_.get<float>("tank.front_armor_factor"),
                       params_.get<float>("tank.rear_armor_factor"));
...
//------------------------------------------------------------------------------
void Tank::activateBoost(bool activate)
{

    // performance optim.
    if(boost_activated_ == activate) return;
    else boost_activated_ = activate;   

    if(activate)
    {
        // store old values
        prev_max_speed_ = max_speed_;
        prev_static_mu_long_acc_ = static_mu_long_acc_;
        
        // set new
        max_speed_ = s_params.get<float>("boost.max_speed");
        static_mu_long_acc_ = s_params.get<float>("boost.static_mu_long_acc");
    }
    else
    {
        max_speed_ = prev_max_speed_;
        static_mu_long_acc_ = prev_static_mu_long_acc_;
    }
}
...
//------------------------------------------------------------------------------
Tank::Tank() :
    target_steer_angle_(0.0f),
    proxy_steer_angle_(0.0f),
    target_yaw_  (0.0f),
    target_pitch_(0.0f),
    proxy_yaw_   (0.0f),
    proxy_pitch_ (0.0f),
    ...
    max_speed_(0.0f),
    boost_activated_(false),
    prev_max_speed_(0.0f),
    prev_static_mu_long_acc_(0.0f)
{
    getObservable()->addObserver(ObserverCallbackFun0(this, &Tank::setNetworkFlagDirty),
                                 HTE_STATE_CHANGED,
                                 &fp_group_);
...
//------------------------------------------------------------------------------
/**
 *  Clone the tank for the replay simulator.
 */
Tank::Tank(const Tank & other, physics::OdeSimulator * sim) :
    Controllable(other, sim),
    target_steer_angle_(other.target_steer_angle_),
    proxy_steer_angle_ (other.proxy_steer_angle_),
    ...
    launcher_pos_(other.launcher_pos_),
    ...
    boost_activated_(other.boost_activated_),
    prev_max_speed_(other.prev_max_speed_),
    prev_static_mu_long_acc_(other.prev_static_mu_long_acc_)
{
    setLocallyControlled(true);
    setLocation(CL_REPLAY_SIM);
...
void Tank::initializeTurret()
{
    ...
    for (int geom=0; geom<(int)target_object_->getGeoms().size(); ++geom)
    {
        ...
        } else if (cur_geom->getName() == "barrel")
        {
            ...
            found_barrel = true;
        } else if (cur_geom->getName() == "launcher")
        {
            launcher_pos_ = cur_geom->getTransform();
            found_launcher = true;
        } else continue;
    ...
    if (!found_barrel || !found_turret || !found_launcher)
    {
        throw Exception("Tank model is missing turret and/or barrel and/or launcher geom");
    }

    // We want relative positions
    launcher_pos_.getTranslation() -= turret_pos_;
    barrel_pos_ -= turret_pos_;
    // Geoms are relative to COG
    turret_pos_ += target_object_->getCog(); 
...
//------------------------------------------------------------------------------
/**
 *  Check for uninitialized weapon slots, create dummies for them.
 */
void Tank::createDummyWeaponSystems()
{
    if (getLocation() == CL_REPLAY_SIM) return;
    ...
    for (unsigned w=0; w<NUM_WEAPON_SLOTS; ++w)
    {
        if (!weapon_system_[w])
        {
            ...
            if (getLocation() == CL_SERVER_SIDE)
            {
                weapon_system_[w] = s_weapon_system_loader.create("WeaponSystemServer");
            } else
            {
                weapon_system_[w] = s_weapon_system_loader.create("WeaponSystemServer");
            }
            
            weapon_system_[w]->init(this, "dummy_weapon_system");
        }
    }
}

...
void GUIScore::setTeams(const std::vector<Team*> & teams)
{
    ...
    if (teams.size() == 1)
    {
        Color col = teams[0]->getColor();
        std::string hex_col = col.getHexString(true);

        // Name & Color teams
        team_name_[0]->setText(teams[0]->getName());
        team_name_[0]->setProperty("TextColours","tl:"+hex_col+" tr:"+hex_col+" bl:"+hex_col+" br:"+hex_col);

        lists_used_.clear();
        lists_used_.push_back(lists_available_[2]); // 2 == big list
    ...
    else if (teams.size() == 2)
    {
        ...
        for(unsigned t=0; t < teams.size(); t++)
        {
            ...
            // Name & Color teams
            team_name_[t]->setText(teams[t]->getName());
            team_name_[t]->setProperty("TextColours",cegui_hex_color);

            lists_used_.push_back(lists_available_[t]);


            // show team scores
            team_score_label_[t]->setVisible(true);
            team_score_value_[t]->setVisible(true);

            team_score_label_[t]->setProperty("TextColours",cegui_hex_color);
            team_score_value_[t]->setProperty("TextColours",cegui_hex_color);
        }
...
void GUIScore::customizeLayout()
{
    for(unsigned c=0; c < NUM_SCORE_LISTS; c++)
    {
        ...
        lists_available_[c]->addColumn("Latency",   PING_COLUMN_ID,     CEGUI::UDim(0.099f,0));
    ...
    spectator_list_->addColumn("Latency",   1,     CEGUI::UDim(0.119f,0));
...
//------------------------------------------------------------------------------
void GUIScore::registerCallbacks()
{

...
HitpointTracker::HitpointTracker(const HitpointTracker & other) :
    ...
    is_shielded_(other.is_shielded_),
...
//------------------------------------------------------------------------------
void HitpointTracker::setShield(void * i)
{
    is_shielded_       = (bool)i;
...
//------------------------------------------------------------------------------
bool HitpointTracker::isShielded() const
{
    return is_shielded_; 
}
...
void HitpointTracker::dealWeaponDamage(int amount, const SystemAddress & damage_dealer, HIT_LOCATION location)
{
    ...
    if (is_shielded_) amount = (int)((float)amount * 0.3);  // XXX test
                                          ...
                                          float factor)
{
    ...
    int damage = (int)(factor*calcCollisionDamage(collision_speed, max_speed));
    if (damage == 0) return;

    addAssistDamage(std::min((unsigned)damage, hitpoints_), damage_dealer);
    setHitpoints((int)hitpoints_ - damage);

    // Disable collision damage, re-enable it after a given time
    collision_damage_enabled_ = false;
...
//------------------------------------------------------------------------------
float HitpointTracker::calcCollisionDamage(float collision_speed, float max_speed)
{
    float speed_percentage;

    if (collision_speed > max_speed)
    {
        speed_percentage = 1.0f;
    } else if (collision_speed < cd_speed_threshold_)
    {
        speed_percentage = 0.0f;
    } else
    {
        speed_percentage = (collision_speed - cd_speed_threshold_) / (max_speed - cd_speed_threshold_);
    }
 
    return cd_min_damage_ + speed_percentage * (cd_max_damage_ - cd_min_damage_);
}

...
//------------------------------------------------------------------------------
bool TankApp::onVersionInfoReceived(Packet * p, bool mismatch)
{
    RakNet::BitStream stream(&p->data[1], p->length-1, false);
    VersionInfo server_version;

    if (!server_version.readFromBitstream(stream))
    {
        s_log << Log::error
              << "Couldn't read version in TankApp::onVersionInfoReceived. Closing connection\n";
    ...
    if (mismatch)
    {
        ...
        if (server_version.type_ != g_version.type_)
        {
            strstr << "This server is not running our game.";
        } else
        {
            strstr << "Version mismatch! "
                   ...
                   << "Server version: "
                   << server_version
                   << ".\nThe latest version can be downloaded from the game homepage.";
        }
    ...
    try
    {
        ...
        while (packet.receive())
        {
            ...
            case ID_CONNECTION_REQUEST_ACCEPTED:
            {
                ...
                network::SetPlayerDataCmd name_cmd(s_params.get<std::string>("client.app.player_name"),
                                                   player_id_,
                                                   session_key_);
            ...
            case ID_DISCONNECTION_NOTIFICATION:
                throw Exception(std::string("You have been disconnected from ") + packet->systemAddress.ToString());
                break;
            case ID_CONNECTION_LOST:
                throw Exception(std::string("Lost connection to ") + packet->systemAddress.ToString());
                break;
            ...
            case VHPI_VERSION_INFO:
                ...
                onVersionInfoReceived(packet, packet_id != VHPI_VERSION_INFO);
            ...
            default:
                std::auto_ptr<NetworkCommandServer> cmd(NetworkCommandServer::createFromPacket(packet_id, packet, interface_));

                // don't accept any commands if we haven't loaded our
                // level yet...
                if (cmd.get() && 
 ...
 *  us yet.
 *
 *  LoadLevel is the first command the server sends to us after
 *  version ack.
 */
...
network::ACCEPT_VERSION_CALLBACK_RESULT TankApp::acceptVersionCallback(const VersionInfo & version)
{
    ...
    if (cmp_version.type_ != version.type_ &&
        g_version.type_ != version.type_)
    {
        return AVCR_TYPE_MISMATCH;
    }

    return (cmp_version.major_ == version.major_ &&
            cmp_version.minor_ == version.minor_) ? AVCR_ACCEPT : AVCR_VERSION_MISMATCH;
}

...
bool                    WeaponSystem::weapon_system_debug_ = false;
...
//------------------------------------------------------------------------------
void WeaponSystem::setCooldownStatus(float f)
{
    heating_ = f;
}
...
//------------------------------------------------------------------------------
const std::string & WeaponSystem::getSection() const
{
    return section_;
}
...
void WeaponSystem::blockFiring()
{
    if (isBlocked())
    {
        s_scheduler.reschedule(task_reset_firing_blocked_, s_params.get<float>(section_ + ".fire_dt"));
    } else
    {

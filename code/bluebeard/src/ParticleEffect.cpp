...
ParticleEffectContainer ParticleEffect::createParticleEffect()
{
    ...
    if (pe_data_.emitter_.shooter_.velocity_scale_ != 0.0f)
    {
        ...
        // need update callback here because of velocity user data
        assert(modular_emitter->getUpdateCallback() == NULL);
        modular_emitter->setUpdateCallback(new VelocityScaleShooterCallback(vss));

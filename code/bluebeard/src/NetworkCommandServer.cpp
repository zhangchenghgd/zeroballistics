...
//------------------------------------------------------------------------------
ReplaceGameObjectCommand::ReplaceGameObjectCommand() :
    replaced_id_(0),
    start_replacing_id_(0)
{
}


//------------------------------------------------------------------------------
ReplaceGameObjectCommand::ReplaceGameObjectCommand(uint16_t object_id,
                                                   uint16_t replacement_start_id,
                                                   const std::string appendix_list) :
    replaced_id_(object_id),
    start_replacing_id_(replacement_start_id),
    appendix_list_(appendix_list)
{
}

//------------------------------------------------------------------------------
void ReplaceGameObjectCommand::execute(PuppetMasterClient * master)
{
#ifndef DEDICATED_SERVER
    master->replaceGameObject(replaced_id_, start_replacing_id_, appendix_list_);
#endif
}

//------------------------------------------------------------------------------
void ReplaceGameObjectCommand::getNetworkOptions(PacketReliability & reliability,
                                                 PacketPriority    & priority,
                                                 unsigned          & channel)
{
    reliability = RELIABLE_ORDERED;
...
//------------------------------------------------------------------------------
void ReplaceGameObjectCommand::writeToBitstream (RakNet::BitStream & stream)
{
    stream.Write((char)TPI_REPLACE_GAME_OBJECT);
    
    stream.Write(replaced_id_);
    stream.Write(start_replacing_id_);

    network::writeToBitstream(stream, appendix_list_);
}


//------------------------------------------------------------------------------
void ReplaceGameObjectCommand::readFromBitstream(RakNet::BitStream & stream)
{
    char packet_id;
    
    stream.Read(packet_id);

    stream.Read(replaced_id_);
    stream.Read(start_replacing_id_);

    network::readFromBitstream(stream, appendix_list_);
}

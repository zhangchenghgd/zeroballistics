...
OdeRigidBody * OdeModelLoader::loadModel(const std::string & name)
{
    ...
    TiXmlNode * lifetime_node = root_handle.FirstChildElement("Lifetime").ToElement();
    if (lifetime_node)
    {
        ret->lifetime_ = getAttributeInt(lifetime_node, "value");
    }

    TiXmlNode * ap_node = root_handle.FirstChildElement("ActivationPoints").ToElement();
    if (ap_node)
    {
        ret->activation_points_ = getAttributeInt(ap_node, "value");
    }

    TiXmlNode * align_cog_node = root_handle.FirstChildElement("AlignCog").ToElement();
    if (align_cog_node)
    {
        ret->align_cog_ = getAttributeBool(align_cog_node, "value");
    }

    TiXmlNode * client_only_node = root_handle.FirstChildElement("ClientSide").ToElement();
    if (client_only_node)
    {
        ret->client_side_only_ = getAttributeBool(client_only_node, "value");
    }
...
OdeGeom * OdeModelLoader::loadShape(const std::string & name, TiXmlNode * shape_node)
{
    ...
    try
    {
        ...
        ret->mass_only_ = getAttributeString(shape_node, "mass_only") == "1";

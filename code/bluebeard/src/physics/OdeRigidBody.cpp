...
OdeRigidBody::OdeRigidBody() :
    ...
    lifetime_(0),
    activation_points_(0),
    align_cog_(false),
    client_side_only_(false)
{
}
...
OdeRigidBody * OdeRigidBody::instantiate(dBodyID id, OdeSimulator * simulator) const
{
    ...
    ret->activation_points_ = activation_points_;
    ret->align_cog_ = align_cog_;
    ret->client_side_only_ = client_side_only_;
    ...
    for (unsigned g=0; g<geom_.size(); ++g)
    {
        ...
        {
            // All bodies start out sleeping in static space for
            // performance reasons.
            new_geom->setSpace(simulator->getStaticSpace());
        }
    }
...
//------------------------------------------------------------------------------
/**
 *  Moves all geoms to new_space. Sensors are ignored.
 */
void OdeRigidBody::changeSpace(OdeCollisionSpace * new_space)
{
    for (std::vector<OdeGeom*>::iterator it = geom_.begin();
         it != geom_.end();
         ++it)
...
//------------------------------------------------------------------------------
void OdeRigidBody::setBelowWater(bool b)
{
    below_water_ = b;
}

//------------------------------------------------------------------------------
bool OdeRigidBody::isBelowWater() const
{
    return below_water_;
}
...
//------------------------------------------------------------------------------
void OdeRigidBody::setActivationPoints(unsigned p)
{
    activation_points_ = p;
}


//------------------------------------------------------------------------------
bool OdeRigidBody::dealActivation(unsigned d)
{
    if (activation_points_ == 0) return false;
    if (activation_points_ <= d) return true;
    activation_points_ -= d;
    return false;
}

//------------------------------------------------------------------------------
unsigned OdeRigidBody::getLifetime() const
{
    return lifetime_;
}
...
//------------------------------------------------------------------------------
bool OdeRigidBody::isClientSideOnly() const
{
    return client_side_only_;
}


//------------------------------------------------------------------------------
bool OdeRigidBody::isAlignCogSet() const
{
    return align_cog_;
}

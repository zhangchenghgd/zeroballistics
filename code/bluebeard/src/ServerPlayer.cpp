...
ServerPlayer::ServerPlayer(const SystemAddress & id) :
    ...
    deque_size_(0),
    ranking_id_ (network::ranking::INVALID_USER_ID),
...
ServerPlayer::ServerPlayer(const ServerPlayer & other) :
    Player             (other),
    ai_player_         (NULL), ///< not copied, due to destr.
...
{
    delete ai_player_;  ///< delete ai_player object here
                        ///< as ServerPlayer is the owner now
}

//------------------------------------------------------------------------------
void ServerPlayer::setAuthData(uint32_t id, uint32_t session_key)
{
    s_log << name_ << " auth data set to "
          << id
          << "; "
          << session_key
          << "\n";
...
//------------------------------------------------------------------------------
uint32_t ServerPlayer::getRankingId() const
{
    return ranking_id_;
}

//------------------------------------------------------------------------------
uint32_t ServerPlayer::getSessionKey() const
{
    return session_key_;
}
...
//------------------------------------------------------------------------------
AIPlayer * ServerPlayer::getAIPlayer()
{
    return ai_player_;
}

//------------------------------------------------------------------------------
void ServerPlayer::setAIPlayer(AIPlayer * aip)
{
    ai_player_ = aip;
}

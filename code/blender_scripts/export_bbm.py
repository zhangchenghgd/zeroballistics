...
import Blender
from Blender import Types, Object, Draw, Scene, Image, Mathutils
...
class Vertex:
    #--------------------------------------------------------
    #  Construct a vertex from 3d coordinates, normal, texture
    #  coordinates (for each UV layer), tangent, and index (this is
    #  the blender vertex index!)
    def __init__(self, v, n, t, tangent, index):
        self.v_ = v
        self.n_ = n
        self.t_ = t
        if tangent:
            self.tangent_   = tangent
            self.bitangent_ = vecCross(self.tangent_, self.n_)
        else:
            self.tangent_   = None
            self.bitangent_ = None
    ...
    def equals(self, v):
        ...
        if self.t_ and v.t_: equal = equal and reduce(lambda a,b: a and b, map(areTexcoordsEqual,self.t_,v.t_),True)
        elif self.t_ is not v.t_: return False
        if self.tangent_ and v.tangent_: equal = equal and areVectorsEqual(self.tangent_, v.tangent_)
        elif self.tangent_ is not v.tangent_: return false
        return equal
...
class MeshInfo:
    ...
    #--------------------------------------------------------
    #
    #  Traverse all vertices from the face and put them into the
    #  vertex array if they aren't already there. mesh is used to
    #  access the different layer's texture coordinates. tangents are
    #  specifically for this face (3 or 4 elements).
    def addFace(self, f, mesh, tangents):
        ...
        for v_number, v in enumerate(f.v):
            ...
            if tangents: cur_tangent = tangents[v_number]
            else: cur_tangent = None
            vertex = Vertex(v.co, n, t, cur_tangent, v.index)
    ...
    def _writeVertices(self, file):
        ...
        if self.vertex_list[0].tangent_:
            writeUnsigned32(file, len(self.vertex_list))
            for vertex in self.vertex_list:
                assert(vertex.tangent_) # Either every vertex has tangents or none.
                writeVector(file, vertex.tangent_)

            writeUnsigned32(file, len(self.vertex_list))
            for vertex in self.vertex_list:
                writeVector(file, vertex.bitangent_)
        else:
...
def writeMeshes(obj, file):
    ...
    # Calculate mesh tangents and transform them into world space
    # If no face has bump mapping active, this is lost work...
    assert(mesh.activeUVLayer and
           mesh.activeUVLayer == mesh.getUVLayerNames()[0])
    tangents = mesh.getTangents()
    inv_world = Mathutils.Matrix(obj.getInverseMatrix())
    tangents_world = []
    for i in range(len(tangents)):
        tangents_world.append([])
        for j in range (len(tangents[i])):
            tangents_world[i].append(inv_world*Mathutils.Vector(tangents[i][j]))
    ...
    for face_ind, f in enumerate(mesh.faces):
        ...
        if mat.bump_map: cur_info.addFace(f, mesh, tangents_world[face_ind])
        else:            cur_info.addFace(f, mesh, None)

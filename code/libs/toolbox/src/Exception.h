...
public:
    ...
    static void set_dump_location(const std::string & path, const std::string & app_name);
    static std::string get_dump_location();
...
private:
    static bool Win32Exception::generateDump(EXCEPTION_POINTERS* pExceptionPointers);

    const char* info_;
    const void* location_;
    unsigned code_;

    static std::string dump_location_;
};

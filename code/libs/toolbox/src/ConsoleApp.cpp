...
void ConsoleApp::handleInputWin32()
{
    ...
    if (_kbhit()) 
    {
        ...
        case 339:  /// Delete key
            {
                COORD coord_screen = { 0, 0 };    /* here's where we'll home the
                                                    cursor */ 
                DWORD console_size;                 /* number of character cells in
                                                    the current buffer */

                GetConsoleScreenBufferInfo( console_wnd, &console_screen_info );
                console_size = console_screen_info.dwSize.X * console_screen_info.dwSize.Y;

                /* fill the entire screen with blanks */ 
                FillConsoleOutputCharacter( console_wnd, (TCHAR) ' ',
                   console_size, coord_screen, written );

                /* get the current text attribute */ 
                GetConsoleScreenBufferInfo( console_wnd, &console_screen_info );

                /* put the cursor at (0, 0) */ 
                SetConsoleCursorPosition( console_wnd, coord_screen );

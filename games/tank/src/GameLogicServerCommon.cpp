...
const float REDUCED_IMPACT_IMPULSE_FACTOR = 0.7;
                                            ...
                                            const physics::CollisionInfo & info)
{
    ...
    bool reduced_impact_impulse = false;
    
    if (hit_object && hit_object->getType() == "Tank")
    {
        Tank * tank = (Tank*)hit_object;

        reduced_impact_impulse = tank->hasInertialDampener();
        ...
        sendWeaponHit(proj_owner, tank->getOwner(),
                      info, type);
    ...
    } else if (hit_object && hit_object->getType() == "Water") // handle water hit different for particle effect
    {
        ...
        if(hit_percentage == 1.0f)
        {
            // increment shots, nothing meaningful is hit
            if (player_score)
            {
                player_score->shots_++;
            }
    ...
    if (hit_object && !hit_object->isStatic())
    {
        ...
        if (hit_percentage == 1.0f)
        {
            ...
            if (reduced_impact_impulse) impulse *= REDUCED_IMPACT_IMPULSE_FACTOR;
...
bool GameLogicServerCommon::tankBodyCollisionCallback(const physics::CollisionInfo & info)
{
    ...
    bool ram_upgrade_active = false;
    if (!equalsZero(collision_velocity))
    {
        v_rel.normalize();
        if (isRamUpgradeActive(tank, other_tank, v_rel))
        {
            ram_upgrade_active = true;
            if (other_tank &&
                !isRamUpgradeActive(other_tank, tank, -v_rel))
            {
                other_tank->dealCollisionDamage(collision_velocity,
                                                other_tank->getMaxSpeed(),
                                                tank->getOwner(),
                                                s_params.get<float>("server.logic.ram_damage_factor"));

                if (other_tank->getHitpoints() <=0)
                {
                    kill(PKT_COLL_DAMAGE, other_tank, tank->getOwner());
                }
            }
        }
    }
    ...
    if (!tank->hasInertialDampener() && !ram_upgrade_active)
    {
                                               ...
                                               COMMAND_SEND_TYPE type)
{
...

    {
        ...
        //update ping for broadcast XXX needs better solution, update ping constantly
        score->ping_ = (uint16_t)(puppet_master_->getPlayer(player_id)->getNetworkDelay() * 1000.0f);

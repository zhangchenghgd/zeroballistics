...
void GUIServerBrowser::registerCallbacks()
{
    ...
    server_list_[INTERNET_LIST]->addObserver(
                            ObserverCallbackFun0(this, &GUIServerBrowser::onNewGameVersionAvailable),
                            network::master::SLE_GREATER_VERSION_AVAILABLE,
                            &fp_group_);
...
//------------------------------------------------------------------------------
void GUIServerBrowser::onNewGameVersionAvailable()
{
    enableFloatingPointExceptions(false);


    notification_area_->setText("There is a new version of Zero Ballistics available. "
                                    "Please download the latest version from the game's homepage.");

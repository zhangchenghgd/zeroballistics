...
void GameLogicClientCommon::init(PuppetMasterClient * master)
{
    ...
    gui_score_.             reset(new GUIScore(master));
    gui_help_.              reset(new GUIHelp());
    gui_upgrade_system_.    reset(new GUIUpgradeSystem(master));
    gui_tank_equipment_.    reset(new GUITankEquipment(master));
...
void GameLogicClientCommon::addPlayer(const Player * player)
{
    ...
    PlayerScore * new_score = score_.addPlayer(player);
    gui_score_->update(score_);

    if (player == puppet_master_->getLocalPlayer())
    {
        spectator_camera_.trackPlayer(player->getId());
        s_console.addVariable("ping", &new_score->ping_, &fp_group_);
...
void GameLogicClientCommon::loadLevel(const std::string & lvl_name)
{
    ...
    hud_.reset(new GameHudTank(puppet_master_));
    hud_->enable(false);
...
void GameLogicClientCommon::onGameObjectAdded(GameObject * object)
{
    ...
    if (rigid_body->getType() == "Projectile")
    {
...
//------------------------------------------------------------------------------
void GameLogicClientCommon::tankExplosion(RakNet::BitStream & args)
{
    uint16_t tank_id;
    args.Read(tank_id);
    ...
    s_log << Log::debug('l')
          << "Tank destroy/explosion command stage "
          << (int)stage
          << " received for tank "
          << tank_id
          << "\n";
    ...
    {
        s_log << Log::warning
              << "Ignoring tankExplosion for invalid tank "
              << tank_id
              << "\n";
        return;
    }
...
void GameLogicClientCommon::weaponHit(RakNet::BitStream & args)
{
    ...
    {
        s_scene_manager.getRootNode()->addChild(s_effect_manager.createEffect(effect_group, pos, n, false));
    }
        ...
        if (puppet_master_->getLocalPlayer()->getId() == id)
        {
            for (unsigned c=0; c<NUM_UPGRADE_CATEGORIES; ++c)
            {
                    ...
                    !upgrade_possible[c])
                {
                    ...
                    {
                        const Color c(0.0f,1.0f,0.0f);
                        puppet_master_->getHud()->addMessage("Upgrade available!", c);
                        puppet_master_->getHud()->addMessage("Press '" +
                                                             s_input_handler.getKeyForFunction("Show Upgrade Screen") +
                                                             "' to display the upgrade screen,", c);
                        puppet_master_->getHud()->addMessage("or press '" + 
                                                             s_input_handler.getKeyForFunction("Upgrade Weapons") + "', '" +
                                                             s_input_handler.getKeyForFunction("Upgrade Armor") + "' or '" +
                                                             s_input_handler.getKeyForFunction("Upgrade Speed") +
                                                             "' to upgrade directly.", c);
                    }
            ...
            {
                puppet_master_->getHud()->appendMessage(" (+" + toString(score->upgrade_points_-cur_upgrade_points) + " UP)");
            }
        }
    }
...
void GameLogicClientCommon::setUpgradeLevel(RakNet::BitStream & args)
{
    ...
    // first check if local player, then get remote player
    Controllable * controllable = NULL;
    if(puppet_master_->getLocalPlayer()->getId() == player_id)
    {
        controllable = puppet_master_->getLocalPlayer()->getControllable();
    }
   
    RemotePlayer * remote_player = puppet_master_->getRemotePlayer(player_id);    
    if(remote_player)
    {
        controllable = remote_player->getControllable();
    }
    ...
    // skip visualisation done below if player is spectator
    if (!controllable) return;

    // determine sum of levels, set visualization accordingly
    unsigned sum=0;
    for (unsigned c=0; c<NUM_UPGRADE_CATEGORIES; ++c)
    {
        sum += score->active_upgrades_[c];
    }
    TankVisual * tv = (TankVisual*)(controllable->getUserData());
    assert(tv);
    tv->setPlayerLevel(sum);
}
...
void GameLogicClientCommon::teamMessage(RakNet::BitStream & args)
{
    ...
    // check if sender is local player (feedback)
    if(player_id == puppet_master_->getLocalPlayer()->getId())
    {
        player = puppet_master_->getLocalPlayer();
    }
    else
    {        
        player = puppet_master_->getRemotePlayer(player_id);
    }
    ...
    } else 
    {
            ...
            info.other_geom_->getName() != "s_mine")
        {
            ...
            if (dot > COLLISION_SOUND_THRESHOLD)
            {
                ...
                if (puppet_master_->getLocalPlayer()->getControllable() == tank) hud_->activateHitMaker(info.pos_);
            }
        }
    }
...
{ 
    ...
    TankVisual * tank_visual = (TankVisual*)(controllable->getUserData());
    assert(tank_visual);
    tank_visual->setPlayerLevel(sum);
}
...
void GameLogicClientCommon::updatePlayerLabel(Player * player)
{
    ...
    if (team)
    {
        controllable_visual->setLabelColor(
            s_params.get<Color>(team->getConfigName() + ".color"));
    }
}
...
{
    if (!spectator_camera_.getTargetControllable()) return;
    if(hud_.get()) hud_->enable(e);
    ((TankVisual*)spectator_camera_.getTargetControllable()->getUserData())->enableInternalView(e);    
}
                                   ...
                                   bool assist)
{
    ...
    {
    case PKT_WEAPON1:
        kill_type_msg = " with Splash Cannon";
        break;
    case PKT_WEAPON2:
        kill_type_msg = " with Armor Piercing";
        break;
    case PKT_WEAPON3:
        kill_type_msg = " with High Impact";
        break;
    case PKT_MINE:
        kill_type_msg = " with a Mine";
        break;
    case PKT_MACHINE_GUN:
        kill_type_msg = " with the Machine Gun";
        break;
    case PKT_COLL_DAMAGE:
        kill_type_msg = " by collision";
        break;
    default:
        kill_type_msg = "";
        break;
    ...
    PlayerScore * local_score = score_.getPlayerScore(puppet_master_->getLocalPlayer()->getId());
    Color color = Color(1.0f, 1.0f, 1.0f);

    if (killed != UNASSIGNED_SYSTEM_ADDRESS)
    {
        // A player was killed.
        if (killer == puppet_master_->getLocalPlayer()->getId())
        {
            ...
            if (player) 
            {
                if(assist)
                {
                    hud_msg = "Kill damage assist bonus received for " + player->getName();
                }else
                {
                    hud_msg = "You neutralized " + player->getName() + kill_type_msg;
                }
            }
        ...
        {            
            ...
            {
                // ... by self
                hud_msg = "You killed yourself.";
            } else
            {
                // .. by other player
                player = puppet_master_->getRemotePlayer(killer);
                if (player) hud_msg = "You were destroyed by " + player->getName() + kill_type_msg;
        ...
        } else
        {
            ...
            if (player2)
            {
                ...
                {
                    // It was a suicide
                    hud_msg = player2->getName() + " committed suicide.";
                } else
                {
                    // It was murder
                    player = puppet_master_->getRemotePlayer(killer);
                    if (player)
                    {
                        hud_msg = player->getName() + " fragged " + player2->getName() + kill_type_msg;
                        color = score_.getPlayerScore(player->getId())->getTeam() == local_score->getTeam() ?
                            Color(0.0f, 0.0f, 1.0f) :
                            Color(1.0f, 0.0f, 0.0f);
                    } else s_log << Log::warning
                                 << player2->getName()
                                 << " has been killed by unknown player "
                                 << killer
                                 << ".\n";
                }
            } else s_log << Log::warning << "Killed player " << killed << " is unknown\n";
        }
    }

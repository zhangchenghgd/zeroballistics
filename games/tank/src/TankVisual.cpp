...
bool WheelVisual::collisionCallback(const physics::CollisionInfo & info)
{
    ...
    penetration_ = std::max(penetration_, radius_ - info.penetration_);
    return false;
}
...
void WheelVisual::update(const Tank * tank, float dt)
{
    ...
    Tank::getTerrainData()->collideRay(info,
                                       tank->getTransform().transformPoint(pos_),
                                       tank->getTransform().getY(),
                                       0.0f,
                                       false);
...
void TankVisual::fireEffect(const std::string & sound)
{
    ...
    {
        sound_pos = s_soundmanager.getListenerInfo().position_;
    }
    
    SoundSource * src = s_soundmanager.playSimpleEffect(sound, sound_pos);

    // This is neccessary to avoid that the firing sound trails behind
    // when moving.
    if (internal_view_)
    {
        src->addUpdateCallback(new SoundSourcePositionAndVelocityUpdater(tank, false));
    }
}
...
void TankVisual::initWheels()
{
    ...
    {
        s_log << Log::error
              << "Less wheel visuals than wheel parameters.\n";
    }


    physics::OdeSimulator * proxy_sim = tank->getProxy()->getSimulator();
    ...
    for(unsigned i=0; i<wheel_.size();++i)
    {
        ...
        physics::OdeRayGeom * ray = new physics::OdeRayGeom(wheel_[i].radius_);
        ray->setName("wheelvisual" + toString(i+1));
        ray->setSensor(true); // or else collision category will be changed...
        ray->setOffset(offset);
        ray->setSpace(proxy_sim->getActorSpace());
        ray->setCollisionCallback(physics::CollisionCallback(&wheel_[i], &WheelVisual::collisionCallback));
        
        tank->getProxy()->addGeom(ray);
        ray->setCategory(CCCC_WHEEL_RAY);


        wheel_[i].pos_ = offset.getTranslation();
        wheel_[i].pos_.y_ -= wheel_[i].radius_;
        
        wheel_[i].model_ = s_scene_manager.insertLocalTransformNode(wheel_node[i]);

        
        
        // Cache parameters.
        if (steer_factor.size() < i+1)
        {
            s_log << Log::error << "No steer factor for wheel " << i+1 << "\n";
        } else
        {
            wheel_[i].steer_factor_ = steer_factor[i];
            wheel_[i].handbraked_   = handbraked[i];
        }
    }
}

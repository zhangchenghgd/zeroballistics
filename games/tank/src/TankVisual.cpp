...
{   
    ...
    float freq = 0.5 + (engine_factor * tank->getGlobalLinearVel().length());
    engine_sound_->setPitch(freq);
}
...
//------------------------------------------------------------------------------
/**
 *  Currently used for mg rotation only.
 */
void TankVisual::enableSecondaryWeaponEffect(bool b)
{
    mg_firing_ = b;
}
...
//------------------------------------------------------------------------------
void TankVisual::enableBoostEffect(bool b)
{
    EnableGroupVisitor g(BOOST_EFFECT_GROUP_NAME, b);
    osg_wrapper_->getOsgNode()->accept(g);    
}
...
void TankVisual::onModelChanged()
{
    ...
    {
        osg::ref_ptr<ParticleEffectNode> inv_effect =
            s_effect_manager.createEffectWithGroup("tank_invincible",
                                                   INVINCIBLE_GROUP_NAME,
                                                   osg_wrapper_->getOsgNode()).first;
    ...
    {
        ParticleEffectNode * n = dynamic_cast<ParticleEffectNode*>(muzzle_v[i]);
        assert(n);
    ...
    {
        wheel_dirt_.push_back(dynamic_cast<ParticleEffectNode*>(dirt_v[w]));
...
//------------------------------------------------------------------------------
void TankVisual::updateShieldEffect()
{
    Tank * tank = (Tank*)object_;

    if(prev_shielded_ != tank->isShielded())
    {
        prev_shielded_ = tank->isShielded();
        EnableGroupVisitor sh(SHIELD_GROUP_NAME, prev_shielded_);
        osg_wrapper_->getOsgNode()->accept(sh);
    }
}

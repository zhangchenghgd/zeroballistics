...
void GUITankEquipment::fillComboboxes()
{
    ...
    ///----------- Primary Weapons -----------------
    itm = new EquipmentListboxItem("Splash Damage", 0, 0, true,
                                   s_params.get<std::string>("splash_cannon.description"));
    equipment_combobox_[0]->addItem(itm);

    itm = new EquipmentListboxItem("Armor Piercing", 1, 1, true, 
                                   s_params.get<std::string>("armor_piercing_cannon.description"));
    equipment_combobox_[0]->addItem(itm);

    itm = new EquipmentListboxItem("Heavy Impact", 2, 2, true, 
                                   s_params.get<std::string>("heavy_impact_cannon.description"));
    equipment_combobox_[0]->addItem(itm);

    /// ----------------- Secondary Weapons -----------------
    itm = new EquipmentListboxItem("Machine Gun", 0, 0, true,
                                   s_params.get<std::string>("default_mg.description"));
    equipment_combobox_[1]->addItem(itm);

    itm = new EquipmentListboxItem("Flamethrower", 1, 1, false,
                                   s_params.get<std::string>("flamethrower.description"));
    equipment_combobox_[1]->addItem(itm);

    itm = new EquipmentListboxItem("Laser", 2, 2, false,
                                   s_params.get<std::string>("laser.description"));
    equipment_combobox_[1]->addItem(itm);

    /// ----------------- First Skills -----------------
    itm = new EquipmentListboxItem("Mines", 0, 0, true,
                                   s_params.get<std::string>("default_mine.description"));
    equipment_combobox_[2]->addItem(itm);

    itm = new EquipmentListboxItem("Ram Bucket", 1, 1, true,
                                   s_params.get<std::string>("skill.ram.description"));
    equipment_combobox_[2]->addItem(itm);

    itm = new EquipmentListboxItem("Guided Missile", 2, 2, false,
                                   s_params.get<std::string>("skill.missile.description"));
    equipment_combobox_[2]->addItem(itm);

    /// ----------------- Second Skills -----------------
    itm = new EquipmentListboxItem("Nano bots", 0, 0, true,
                                   s_params.get<std::string>("skill.nano_bots.description"));
    equipment_combobox_[3]->addItem(itm);

    itm = new EquipmentListboxItem("Shield", 1, 1, false,
                                   s_params.get<std::string>("skill.shield.description"));
    equipment_combobox_[3]->addItem(itm);

    itm = new EquipmentListboxItem("Boost", 2, 2, false,
    ...
    for(unsigned i=0; i < NUM_EQUIPMENT_SLOTS; i++)
    {
        for(size_t item = 0; item < equipment_combobox_[i]->getItemCount(); item++)
        {
            ...
            std::string equip_code = EQUIPMENT_SLOTS[i] + toString(equip_item->equipment_value_);
...
bool GUITankEquipment::clickedOkBtn(const CEGUI::EventArgs& e)
{
    ...
    if(current_combo_editbox_item)
    {
        ...
        equipment.push_back(item->equipment_value_);
    }
    else
    {
    ...
    if(item)
    {
        ...
        for(unsigned i=0; i < NUM_EQUIPMENT_SLOTS; i++)
        {
            if(combobox == equipment_combobox_[i])
            {
                ...
                {                    
                    showPreviewImage(preview_images_[1], EQUIPMENT_SLOTS[i] + toString(item->equipment_value_) +
                                                         accepted_comboboxes_[1] );
                }
                ...
                {
                    showPreviewImage(preview_images_[1], accepted_comboboxes_[0] +
                                                         EQUIPMENT_SLOTS[i] + toString(item->equipment_value_));
                ...
                if(i == 2) // first skill
                {
                    ...
                    showPreviewImage(preview_images_[0], EQUIPMENT_SLOTS[i] + toString(item->equipment_value_) +
                                                         accepted_comboboxes_[3] );
    ...
    item = dynamic_cast<EquipmentListboxItem *>(combobox->getSelectedItem());
        ...
        for(unsigned i=0; i < NUM_EQUIPMENT_SLOTS; i++)
        {
            if(combobox == equipment_combobox_[i])
            {
                ...
                if(item)
                {
                    accepted_comboboxes_[i] = EQUIPMENT_SLOTS[i] + toString(item->equipment_value_);
                }
                ...
                if(item && !item->available_) 
                {
                    ...
                    accepted_comboboxes_[i] = EQUIPMENT_SLOTS[i] + toString(new_selection->equipment_value_);
                }

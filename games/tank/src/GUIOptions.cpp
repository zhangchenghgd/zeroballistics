    ...
    capturing_input_(NULL)
{
    ...
    // disable water combobox on ati cards
    if(s_scene_manager.getOpenGlRenderer() == "ati")
    {
        water_quality_combobox_->disable();
    }
    else
    {
        water_quality_combobox_->enable();
    }
...
void GUIOptions::fillComboboxes()
{
    ...
    res_combobox_->addItem(itm);
    itm = new ResolutionListboxItem("1280x800", 2, 1280, 800);
    res_combobox_->addItem(itm);
    itm = new ResolutionListboxItem("1280x1024", 3, 1280, 1024);
    res_combobox_->addItem(itm);
    itm = new ResolutionListboxItem("1680x1050", 4, 1680, 1050);
    res_combobox_->addItem(itm);
    ...
    itm = new WaterQualityListboxItem("High", 1, 1);
    water_quality_combobox_->addItem(itm);
...
bool GUIOptions::loadOptions()
{
    ...
    // shadows enabled
    shadows_enabled_checkbox_->setSelected(s_params.get<bool>("client.shadows.enabled"));
    ...
    // Shader Quality
    unsigned shader_quality = s_params.get<unsigned>("client.graphics.shader_quality");
    ...
    {
        water_quality = (unsigned)s_params.get<bool>("client.graphics.reflections");
    }
...
bool GUIOptions::saveOptions()
{
    ...
    if(current_combo_editbox_item)
    {
        ...
        s_params.set<bool>("client.graphics.reflections", (bool)water_quality_item->water_quality_);
    }

...
const unsigned NUM_WEAPON_SLOTS = 3;
...
class Tank : public Controllable, public HitpointTracker
{
 public:
    ...
    virtual Tank * clone(physics::OdeSimulator * sim) { return new Tank(*this, sim); }
    ...
    float getMainGunReloadTime() const;
    void setMainGunReloadTime(float t);
    ...
    bool hasMineProtection()   const;
    bool hasInertialDampener() const;
 ...
 protected:
    ...
    float steer_angle_; ///< Steering angle for all wheels. Gets
    ...
    /// Firing the main gun has a major influence on the tank by
    /// applying recoil force. This requires the reload time to become
    /// member of client side predicted state variables.
    float main_reload_time_;
    ...
    WeaponSystem * weapon_system_[NUM_WEAPON_SLOTS];

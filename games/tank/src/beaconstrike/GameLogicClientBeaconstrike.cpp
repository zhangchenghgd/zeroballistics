                                         ...
                                         bool assist)
{
    ...
    if (kill_type == PKT_UNKNOWN1)
    {
        ...
        } else
        {
            ...
            if (player) hud_msg = player->getName() + " has destroyed a beacon!";
        }
...
void GameLogicClientBeaconstrike::beaconExplosion(RakNet::BitStream & args)
{
    ...
    {
        puppet_master_->getHud()->addMessage("Beacon lost.", Color(1,0.7,0,1));
    }    
}
...
void GameLogicClientBeaconstrike::stageConquered(RakNet::BitStream & args)
{
    ...
    puppet_master_->getHud()->addMessage("A base has been conquered.", Color(1,0.7,0,1));
...
void GameLogicClientBeaconstrike::onBeaconHealthChanged(Observable* observable, unsigned event)
{
    ...
    if (beacon->getTeamId() == score_.getTeamId(puppet_master_->getLocalPlayer()->getId()))
    {
        ...
        {
        case BE_HEALTHY:
            puppet_master_->getHud()->addMessage("One of our beacons has regenerated.", Color(0,1,0,1));
            hud_->getMinimap()->enableFlash(beacon, false);
            break;
            
        case BE_UNDER_ATTACK:
            puppet_master_->getHud()->addMessage("One of our beacons is losing health.", Color(1,1,0,1));
            hud_->getMinimap()->enableFlash(beacon, true);
            break;
            
        case BE_HEALTH_CRITICAL:
            puppet_master_->getHud()->addMessage("One of our beacons is close to destruction.", Color(1,0,0,1));
            break;
        default:
            assert(false);
        }

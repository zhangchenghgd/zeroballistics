...
{
    // TODO check why this triggers with bots, fix it
...
void Score::removePlayer(const SystemAddress & id)
{
    ...
    if (player_score_.find(id) == player_score_.end()) return;
...
//------------------------------------------------------------------------------
/**
 *  returns the number of players for the given team
 */
unsigned Score::getTeamSize(TEAM_ID tid) const
{    
    unsigned ret = 0;
    for (std::map<SystemAddress, PlayerScore>::const_iterator it = player_score_.begin();
         it != player_score_.end();
    ...
    {
        if (it->second.getTeam() == NULL) continue;
        if (it->second.getTeam()->getId() == tid) ret++;
    }
